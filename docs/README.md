...
| File | Description |
|---|---|
| [`architecture/ARCHITECTURE-FINAL.md`](./architecture/ARCHITECTURE-FINAL.md) | **This is the architecture. Start here.** Five engines, clean. Document intelligence → student model (DKT-GAT + FSRS + IRT + SIF) → scheduler (ZPD gradient, challenge manager, interference, interleaving, deadline compression) → content pipeline (format reasoning, generation-first SRE, dialogue layer with confusion classification, constraint enforcement) → population flywheel. Full session walkthrough. Honest compression estimate. School integration path. Final table showing what every system decides and what the LLM does (one row). |
| [`architecture/ARCHITECTURE-V5.md`](./architecture/ARCHITECTURE-V5.md) | Previous iteration. Superseded by FINAL. |
